...
func TestTableNoModulecalls(t *testing.T) {
	assert := assert.New(t)
	settings := testutil.Settings().With(&print.Settings{
		ShowHeader:       true,
		ShowInputs:       true,
	...
	expected, err := testutil.GetExpected("markdown", "table-NoModulecalls")
	assert.Nil(err)
...
func TestTableOnlyModulecalls(t *testing.T) {
	assert := assert.New(t)
	settings := testutil.Settings().With(&print.Settings{
		ShowHeader:       false,
		ShowInputs:       false,
	...
	expected, err := testutil.GetExpected("markdown", "table-OnlyModulecalls")
	assert.Nil(err)
